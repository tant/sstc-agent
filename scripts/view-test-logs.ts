...
async function viewTestLogs() {
	...
	if (fs.existsSync(conversationLogsDir)) {
		...
		if (conversationLogs.length > 0) {
			...
			console.log(
				`   cat ${path.join("tests", "logs", "conversation-logs", conversationLogs[0])}`,
			);
		}
	}
	...
	if (fs.existsSync(testResultsDir)) {
		...
		if (testResults.length > 0) {
			...
			console.log(
				`   cat ${path.join("tests", "logs", "test-results", testResults[0])}`,
			);
		}
	}
	...
	console.log("\n📊 Quick commands:");
	console.log(
		"  List all conversation logs:  ls -la tests/logs/conversation-logs/",
	);
	console.log("  List all test results:       ls -la tests/logs/test-results/");
	console.log(
		"  View latest conversation:    tail -n 20 tests/logs/conversation-logs/*.json",
	);
	console.log(
		"  View latest test result:     tail tests/logs/test-results/*.json",
	);
			...
			if (testResultFiles.length > 0) {
				testResultFiles.forEach((result) => {
					...
					if (!result.passed && result.errorMessage) {
						console.log(
							`    Error: ${result.errorMessage.substring(0, 100)}...`,
						);
					}
				});
			} else {
	...
	console.log(
		'  View all conversation logs:    find tests/logs/conversation-logs -name "*.json" -exec cat {} \\;',
	);
	console.log(
		'  Search for specific keywords:  grep -r "SSD" tests/logs/conversation-logs/',
	);
	console.log(
		"  Count total conversations:     ls tests/logs/conversation-logs/*.json | wc -l",
	);
	console.log(
		'  View conversation by test:     find tests/logs/conversation-logs -name "*test-name*" -exec cat {} \\;',
	);
}
