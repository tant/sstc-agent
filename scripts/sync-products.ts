...
const PRODUCT_SCHEMA: {
	...
	{ name: "model", type: "TEXT", value: (p) => p.Model ?? null },
	{
		name: "price",
		type: "REAL",
		value: (p) => (p.Giá ? parseFloat(p.Giá.replace(/[^\d.]/g, "")) : null),
	},
	...
	{
		name: "comparison_with_other_models",
		type: "TEXT",
		value: (p) => p["Comparison with Other Models"] ?? null,
	},
	{
		name: "upsell_suggestions",
		type: "TEXT",
		value: (p) => p["Upsell Suggestions"] ?? null,
	},
	{ name: "sales_pitch", type: "TEXT", value: (p) => p["Sales Pitch"] ?? null },
	{ name: "bundles", type: "TEXT", value: (p) => p.Bundles ?? null },
	{
		name: "suitable_applications",
		type: "TEXT",
		value: (p) => p["Suitable Applications"] ?? null,
	},
	...
	{ name: "category", type: "TEXT", value: (p) => p["Loại sản phẩm"] ?? null },
	{
		name: "compatible_cpu",
		type: "TEXT",
		value: (p) => p["Tương thích CPU"] ?? null,
	},
	{
		name: "linked_system_model",
		type: "TEXT",
		value: (p) => p["Linked System Model"] ?? null,
	},
	...
	{ name: "usp", type: "TEXT", value: (p) => p.USP ?? null },
	{
		...
		type: "TEXT",
		value: (p) => p.Recommended_Use ?? null,
	},
	{
		name: "benchmark_score",
		type: "TEXT",
		value: (p) => p["Benchmark Score (Read/Write MB/s)"] ?? null,
	},
	{
		name: "power_consumption",
		type: "TEXT",
		value: (p) => p["Power Consumption (W)"] ?? null,
	},
	{ name: "tdp", type: "TEXT", value: (p) => p.TDP ?? null },
	{
		name: "price_per_performance",
		type: "TEXT",
		value: (p) => p["Price per Performance"] ?? null,
	},
	{
		name: "future_proofing",
		type: "TEXT",
		value: (p) => p["Future Proofing"] ?? null,
	},
	{
		name: "benchmarks_passmark",
		type: "TEXT",
		value: (p) => p["Benchmarks (PassMark)"] ?? null,
	},
	{
		name: "power_supply",
		type: "TEXT",
		value: (p) => p["Power Supply (W)"] ?? null,
	},
	{ name: "level", type: "TEXT", value: (p) => p.Level ?? null },
	{
		name: "system_model",
		type: "TEXT",
		value: (p) => p["System Model"] ?? null,
	},
	{
		name: "barebone_linked_sku",
		type: "TEXT",
		value: (p) => p["Barebone Linked SKU"] ?? null,
	},
	{
		name: "linked_sku_ram",
		type: "TEXT",
		value: (p) => p["Linked SKU RAM"] ?? null,
	},
	{
		name: "linked_sku_ssd",
		type: "TEXT",
		value: (p) => p["Linked SKU SSD"] ?? null,
	},
	{
		name: "gia_tong",
		type: "REAL",
		value: (p) =>
			p["Giá Tổng"] ? parseFloat(p["Giá Tổng"].replace(/[^\d.]/g, "")) : null,
	},
	{ name: "total_specs", type: "TEXT", value: (p) => p["Total Specs"] ?? null },
	{
		name: "performance_benchmark",
		type: "TEXT",
		value: (p) => p["Performance Benchmark"] ?? null,
	},
	{
		name: "customer_reviews_rating",
		type: "TEXT",
		value: (p) => p["Customer Reviews/Rating"] ?? null,
	},
	{
		name: "environmental_impact",
		type: "TEXT",
		value: (p) => p["Environmental Impact"] ?? null,
	},
	{
		name: "benchmark_gaming_fps",
		type: "TEXT",
		value: (p) => p["Benchmark Gaming (FPS)"] ?? null,
	},
	{
		name: "weight_dimensions",
		type: "TEXT",
		value: (p) => p["Weight/Dimensions"] ?? null,
	},
	{
		name: "future_upgrade_path",
		type: "TEXT",
		value: (p) => p["Future Upgrade Path"] ?? null,
	},
	{
		name: "noise_level_db",
		type: "TEXT",
		value: (p) => p["Noise Level (dB)"] ?? null,
	},
...
function validateProduct(product: any, fileName: string): ProductData {
	if (!product.SKU || String(product.SKU).trim() === "") {
		throw new Error(
			`Missing or empty SKU in ${fileName} for product: ${product["Tên sản phẩm"] || "Unknown"}`,
		);
	}
	if (
		!product["Tên sản phẩm"] ||
		String(product["Tên sản phẩm"]).trim() === ""
	) {
		throw new Error(
			`Missing or empty product name in ${fileName} for SKU: ${product.SKU}`,
		);
	}
...
async function prepareLibSQL(db: Client): Promise<void> {
	console.log(
		"🗑️  Preparing LibSQL database (dropping and recreating table)...",
	);
	await db.execute("DROP TABLE IF EXISTS products;");

	const columnDefs = PRODUCT_SCHEMA.map(
...
async function syncToLibSQL(
	...
	console.log(
		`🚀 Starting batch sync of ${products.length} products to LibSQL...`,
	);
...
async function syncToChroma(
	...
	console.log(
		`🗑️  Preparing ChromaDB (deleting and recreating index "${indexName}")...`,
	);
	...
	try {
		...
		console.log(
			`✅ ChromaDB index "${indexName}" created with dimension ${dimension}.`,
		);
	} catch (e) {
...
async function main() {
	...
	try {
		...
		const embedder_model_id =
			process.env.EMBEDDER_MODEL ?? "text-embedding-3-small";

		if (!embedder_base_url) {
			throw new Error(
				"EMBEDDER_BASE_URL is not set in .env file. Please check your .env file.",
			);
		}

		console.log(
			`[Embedding Provider] Initializing with URL: ${embedder_base_url}`,
		);
		...
		console.log(
			`[Chroma Vector] Initializing with host: ${chromaHost} port: ${chromaPort} ssl: ${chromaSsl}`,
		);
