	...
	try {
		...
		const maiResponse = await maiSale.generate(
			[
				{
					role: "user",
					content: "Tôi muốn mua case máy tính để build một PC gaming",
				},
			],
			{},
		);
		...
		console.log(
			"📝 Mai response preview:",
			`${maiResponse.text.substring(0, 100)}...`,
		);
		...
		console.log(
			"📝 Barebone response preview:",
			`${bareboneResponse.text.substring(0, 100)}...`,
		);
		...
		console.log(
			"📝 Coordination response preview:",
			`${coordinationTest.substring(0, 100)}...`,
		);
