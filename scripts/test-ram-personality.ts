...
async function testRAMPersonality() {
  ...
  try {
    ...
    const checks = [
      ...
      { term: 'nhân viên kinh doanh', description: 'Mentions salesperson role' },
    ...
    for (const check of checks) {
      const found = instructions.toLowerCase().includes(check.term.toLowerCase());
      console.log(`   ${found ? '✅' : '❌'} ${check.description}: "${check.term}" ${found ? 'FOUND' : 'NOT FOUND'}`);
    ...
    if (passedChecks >= checks.length * 0.8) {
      console.log('✅ RAM agent personality well-aligned with SSTC business context');
    } else {
      console.log('⚠️  RAM agent personality needs further alignment with SSTC business context');
    }
    ...
    try {
      const response = await ramAgent.generate([{ role: 'user', content: testQuery }]);
