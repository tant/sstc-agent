...
describe('Kiểm tra Mai Agent với Barebone Specialist', () => {
  it('nên có thể phối hợp ngầm với barebone specialist', async () => {
    ...
    const customerMessage = "Tôi muốn mua case máy tính để build một PC gaming";
  ...
  it('nên có thể xử lý yêu cầu về barebone mà không tạo sản phẩm hư cấu', async () => {
    const customerMessage = "Tôi cần một case mid-tower cho PC gaming với budget khoảng 2 triệu";
  ...
  it('nên có thể phối hợp với các specialist khác', async () => {
    ...
    const hasCoordinationMethod = coordinationMethods.some(method => 
      typeof (maiSale as any)[method] === 'function'
    );
