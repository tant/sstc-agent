...
export class UnifiedMemoryManager {
  ...
  private constructor() {
    ...
    console.log('✅ [UnifiedMemory] Unified Memory Manager initialized successfully');
  }
  ...
  public async getUserChatHistory(
    ...
    if (cached) {
      console.log(`🔍 [UnifiedMemory] Returning cached history for user: ${userId}`);
      return cached;
    }
    ...
    try {
      ...
      for (const channel of channels) {
        try {
          ...
          if (memoryResult.uiMessages && memoryResult.uiMessages.length > 0) {
            results.push(...memoryResult.uiMessages.map(msg => ({
              ...msg,
              channel: channel,
              threadId: threadId
            })));
          ...
          console.warn(`⚠️ [UnifiedMemory] No history found for ${channel}_user_${userId}:`, error instanceof Error ? error.message : String(error));
        }
      }
      ...
      console.log(`✅ [UnifiedMemory] Found ${results.length} messages for user: ${userId}`);
      return results;

    } catch (error) {
      console.error(`❌ [UnifiedMemory] Error querying history for ${userId}:`, error);
      return [];
    }
  }
  ...
  public async hasUserBeenGreeted(userId: string): Promise<boolean> {
    try {
      console.log(`👋 [GreetingControl] Checking greeting status for user: ${userId}`);

      // Get recent chat history to check if user has been greeted
      const recentMessages = await this.getUserChatHistory(userId, { limit: 5 });

      if (recentMessages.length < 2) {
        console.log('👋 [GreetingControl] Too few messages - assuming new user needs greeting');
        return false;
      }
      ...
      console.log(`👋 [GreetingControl] Greeting status for ${userId}: ${hasGreeting ? 'Already greeted' : 'Needs greeting'}`);

      return hasGreeting;

    } catch (error) {
      console.error(`❌ [GreetingControl] Error checking greeting status for ${userId}:`, error);
   ...
   */
  public async markUserAsGreeted(userId: string, agentType: string, channel: string): Promise<void> {
    try {
      console.log(`✅ [GreetingControl] Marking user ${userId} as greeted by ${agentType}`);
      ...
      console.log(`✅ [GreetingControl] Successfully marked greeting for ${userId}`);

    } catch (error) {
      console.error(`❌ [GreetingControl] Error marking greeting for ${userId}:`, error);
      throw error;
    }
  }
  ...
  public async updateUserProfile(
    ...
    try {
      console.log(`📝 [UnifiedMemory] Updating profile for user: ${userId}`, updates);
      ...
      console.log('✅ [UnifiedMemory] User profile would be updated when Memory API is clarified');

      // Clear cache for this user
      this.cache.clear();

    } catch (error) {
      console.error(`❌ [UnifiedMemory] Error updating profile for ${userId}:`, error);
      throw error;
    }
  }
  ...
  public async storeMessage(
    ...
    try {
      console.log(`💾 [UnifiedMemory] Storing message for ${role} in ${channel}_user_${userId}`);
      ...
      console.error(`❌ [UnifiedMemory] Error storing message for ${userId}:`, error);
    }
  }
   ...
   */
  public async getUserAnalytics(userId: string): Promise<ConversationAnalytics | null> {
    try {
      console.log(`📊 [UnifiedMemory] Generating analytics for user: ${userId}`);
      ...
      for (let i = 0; i < history.length; i++) {
        ...
        if (msg.metadata?.agentType) {
          agentInteractions[msg.metadata.agentType] = (agentInteractions[msg.metadata.agentType] || 0) + 1;
        }

        // Count channels
        if (msg.channel) {
          channelInteractions[msg.channel] = (channelInteractions[msg.channel] || 0) + 1;
        }

        // Count intents
        if (msg.metadata?.intent) {
          intentBreakdown[msg.metadata.intent] = (intentBreakdown[msg.metadata.intent] || 0) + 1;
        }
      }

      const conversationDuration = history.length > 1 ?
        (new Date(history[0].timestamp).getTime() - new Date(history[history.length - 1].timestamp).getTime()) / 1000 / 60 : // minutes
        0;
      ...
      return {
        ...
        agentInteractions,
        channelInteractions,
        avgResponseTime: messagePairs > 0 ? totalResponseTime / messagePairs : 0,
      ...
      console.error(`❌ [UnifiedMemory] Error getting analytics for ${userId}:`, error);
      return null;
    }
  }
  ...
  public async resetUserMemory(userId: string): Promise<void> {
    try {
      ...
      console.error(`❌ [UnifiedMemory] Error resetting memory for ${userId}:`, error);
      throw error;
    }
  }
