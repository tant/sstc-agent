...
export class CentralMessageProcessor {
	...
	async processMessage(message: NormalizedMessage): Promise<ProcessedResponse> {
		...
		setTimeout(
			() => {
				console.log("🗑️ [Processor] Removing message from processed cache", {
					messageKey,
				});
		...
		try {
			...
			console.log(
				"🔄 [Processor] Creating workflow run for message processing",
			);
			...
			const workflowInput = {
				inputData: {
					channelId: message.channel.channelId as
						| "telegram"
						| "whatsapp"
						| "web"
						| "zalo",
			...
			console.log("📤 [Processor] Starting workflow with input:", {
				...
				attachmentCount:
					workflowInput.inputData.message.attachments?.length || 0,
			});
			...
			if (workflowResult.status === "success") {
				...
				console.log(
					"✅ [Processor] Message processed successfully via workflow",
					{
				...
				const responseContent =
					result.response || "Xin lỗi, tôi không thể xử lý yêu cầu của bạn.";
				...
				return {
					...
					metadata: {
						error:
							workflowResult.error?.message || String(workflowResult.error),
				...
				console.error(
					"❌ [Processor] Workflow processing failed with unknown status:",
					{
						status: workflowResult.status,
					},
				);
