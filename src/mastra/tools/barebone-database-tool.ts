...
const bareboneSearchInputSchema = z.object({
  ...
  caseSize: z.enum(['mini-tower', 'mid-tower', 'full-tower', 'small-form-factor', 'micro-atx']).optional().describe('Preferred case size'),
  motherboardFormFactor: z.enum(['ATX', 'Micro-ATX', 'Mini-ITX', 'E-ATX']).optional().describe('Preferred motherboard form factor'),
  supportedSocket: z.enum(['LGA1700', 'LGA1200', 'AM5', 'AM4', 'LGA2066']).optional().describe('Supported CPU socket'),
  ...
  coolingType: z.enum(['air', 'liquid', 'hybrid']).optional().describe('Preferred cooling type'),
  aestheticsStyle: z.enum(['gaming', 'minimalist', 'professional', 'rgb']).optional().describe('Aesthetics style preference'),
  motherboardCompatibility: z.string().optional().describe('Specific motherboard compatibility requirement')
});
...
export const bareboneDatabaseTool = createTool({
  ...
  description: 'Search SSTC barebone system database and return structured data for specialist agents',
    ...
    const mockResults = [
      {
        ...
        keyFeatures: ['Excellent airflow', 'Cable management', 'Tempered glass side panel'],
