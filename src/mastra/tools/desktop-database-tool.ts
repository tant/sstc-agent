...
import type { DesktopSpecialistData } from '../core/models/specialist-data-models';
...
const desktopSearchInputSchema = z.object({
  ...
  }).optional().describe('Budget range for filtering'),
  useCase: z.enum(['gaming', 'content-creation', 'office', 'professional']).optional().describe('Intended use case'),
  ...
  storageType: z.enum(['SSD', 'NVMe', 'HDD']).optional().describe('Preferred storage type'),
  storageCapacity: z.number().optional().describe('Required storage capacity in GB'),
  caseSize: z.enum(['mini-tower', 'mid-tower', 'full-tower', 'small-form-factor']).optional().describe('Preferred case size'),
  coolingPreference: z.enum(['air', 'liquid']).optional().describe('Cooling solution preference'),
  performanceLevel: z.enum(['budget', 'mid-range', 'high-end']).optional().describe('Performance level preference')
});
...
export const desktopDatabaseTool = createTool({
  ...
  description: 'Search SSTC desktop PC configuration database and return structured data for specialist agents',
    ...
    const mockResults = [
      {
        ...
        keyFeatures: ['High performance gaming', 'Future upgradeable', 'RGB lighting'],
        useCases: ['gaming', 'content-creation'],
        description: 'High-performance gaming desktop with Intel i7 and DDR5 RAM'
      }
    ];
