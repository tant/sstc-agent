...
const INTENT_PATTERNS = {
  purchase: {
    keywords: ['mua', 'buy', 'purchase', 'get', 'order', 'dat hang', 'gia', 'price', 'tim', 'looking for', 'can i buy', 'how much'],
    confidenceBoost: 0.8,
  },
  warranty: {
    keywords: ['bao hanh', 'warranty', 'repair', 'sua chua', 'fix', 'broken', 'hong', 'khong hoat dong', 'not working', 'error'],
    confidenceBoost: 0.9,
  },
  technical: {
    keywords: ['technical', 'specification', 'specs', 'compatibility', 'tương thich', 'setup', 'install', 'setup'],
    confidenceBoost: 0.6,
  },
  general: {
    keywords: ['what', 'how', 'where', 'when', 'giup', 'help', 'support', 'consult', 'tu van', 'can ban tell me'],
    confidenceBoost: 0.3,
  }
};
...
const clarifyIntentOutputSchema = z.object({
  ...
  suggestedIntent: z.enum(['purchase', 'warranty', 'technical', 'general', 'unclear']),
...
export const clarifyIntentTool = createTool({
  id: 'clarify-intent',
  description: 'Analyze user message to determine intent and provide clarification when needed',
  inputSchema: clarifyIntentInputSchema,
  outputSchema: clarifyIntentOutputSchema,
  execute: async (context) => {
    const { message, currentIntent, chatHistory, previousAttempts = 0 } = (context as any);
    ...
    let score = { purchaseScore: 0, warrantyScore: 0, technicalScore: 0, generalScore: 0 };

    // Analyze message against intent patterns
    ...
    for (const [intentKey, intentData] of Object.entries(INTENT_PATTERNS)) {
      ...
      // Cap individual scores at 1.0 and apply to score object
    ...
    if (chatHistory && chatHistory.length > 0) {
      ...
      for (const chatMsg of recentMessages) {
        ...
        if (chatMsg.role === 'assistant' && chatMsg.content.includes('bảo hành')) {
    ...
    const suggestedIntent = Object.keys(intentScores)[Object.values(intentScores).indexOf(maxScore)] as keyof typeof intentScores;
    ...
    const needsClarification = confidence < 0.6 || (maxScore - Object.values(intentScores).sort((a, b) => b - a)[1]) < 0.2;
    ...
    if (needsClarification && !maxAttemptsReached) {
      ...
      const questions = CLARIFICATION_QUESTIONS[suggestedIntent] ||
                       CLARIFICATION_QUESTIONS.general;
      clarificationQuestion = questions[Math.floor(Math.random() * questions.length)];
      ...
      response = 'Bài xin lỗi vì tôi chưa thể hiểu rõ nhu cầu của bạn. Hãy để tôi chuyển bạn đến đội ngũ chăm sóc khách hàng chuyên nghiệp của SSTC để được hỗ trợ trực tiếp nhé!';
      shouldEscalate = true;
      ...
      switch (suggestedIntent) {
        case 'purchase':
          response = 'Tôi hiểu bạn đang quan tâm đến việc mua sắm sản phẩm. Để được tư vấn chi tiết nhất, tôi sẽ chuyển bạn đến đội ngũ kinh doanh nhé!';
          break;
        case 'warranty':
          response = 'Tôi thấy bạn có vấn đề cụ thể với sản phẩm cần được khắc phục. Để được hỗ trợ về bảo hành, tôi sẽ chuyển bạn sang đội ngũ kỹ thuật nhé!';
          break;
        case 'technical':
          response = 'Bạn có vấn đề kỹ thuật cần được giải quyết. Tôi sẽ chuyển bạn đến đội ngũ hỗ trợ kỹ thuật để được tư vấn cụ thể!';
          break;
        default:
          response = 'Tôi sẽ chuyển bạn đến đội ngũ chăm sóc khách hàng SSTC để được hỗ trợ tốt nhất!';
      }
