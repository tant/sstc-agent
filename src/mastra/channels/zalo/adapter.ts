...
export class ZaloChannelAdapter implements ChannelAdapter {
  ...
  async start(): Promise<void> {
    try {
      console.log('🔍 [Zalo] Logging in with provided credentials', {
        ...
        cookieLength: typeof this.config.cookie === 'string' ? this.config.cookie.length : 'object',
  ...
  private async handleZaloMessage(zaloMessage: any): Promise<void> {
    ...
    try {
      ...
      if (!this.validateMessage(normalizedMessage)) {
        ...
        await this.sendResponseViaApi('Xin lỗi, tôi không thể xử lý tin nhắn của bạn. Vui lòng thử lại.', normalizedMessage);
        return;
      }
   ...
   */
  private async sendResponseViaApi(message: string, originalMessage: any): Promise<void> {
    const threadId = originalMessage.threadId;
    try {
