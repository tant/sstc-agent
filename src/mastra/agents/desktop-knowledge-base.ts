...
export class DesktopKnowledgeBase {
  ...
  async initialize(): Promise<void> {
    ...
    console.log("📚 [DesktopKnowledgeBase] Loading all desktop configurations...");
    try {
      ...
      if (toolResult.specialistData?.recommendations) {
        this.configs = toolResult.specialistData.recommendations.map(
          (rec) => ({
  ...
  getStatistics(): {
    ...
    const avgPrice =
      this.configs.reduce((sum, p) => sum + p.price, 0) /
      this.configs.length;
  ...
  searchDesktops(criteria: SearchCriteria): DesktopConfigInfo[] {
    ...
    if (criteria.budget?.min !== undefined) {
      results = results.filter((config) => config.price >= criteria.budget!.min!);
    }

    if (criteria.budget?.max !== undefined) {
      results = results.filter((config) => config.price <= criteria.budget!.max!);
    }
    ...
    if (criteria.cpuBrand) {
      results = results.filter((config) =>
        config.cpuModel.toLowerCase().includes(criteria.cpuBrand!.toLowerCase()),
      );
    }

    if (criteria.ramCapacity !== undefined) {
      results = results.filter((config) => {
        const ramCapacity = parseInt(config.ramConfiguration.capacity);
    ...
    if (criteria.storageCapacity !== undefined) {
      results = results.filter((config) => {
        const storageCapacity = parseInt(config.storageConfiguration.capacity);
    ...
    if (criteria.coolingPreference) {
      results = results.filter((config) =>
        config.coolingSolution.toLowerCase().includes(criteria.coolingPreference!),
      );
    }
