...
const personalityMatch = content.match(
	/const EMBEDDED_PERSONALITY = `([\s\S]*?)`/,
);
...
console.log(
	`📊 Conversion Rate: ${characterCount} chars ≈ ${tokenEstimate} tokens (0.25 ratio)`,
);
console.log("");
...
if (characterCount <= 3200) {
	...
	console.log(
		"⚠️  KHÁC BIỆT! Character count differs significantly from estimate",
	);
	console.log(`   Estimated: 3200 chars | Actual: ${characterCount} chars`);
	console.log(
		`   Difference: ${Math.abs(3200 - characterCount)} chars (${Math.round(Math.abs(3200 - characterCount) / 32)}%)`,
	);
}
